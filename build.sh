...
if ask "Do you want to clean directories?" N ; then do_cleanup; fi

if ask "Do you want to install build dependencies?" $([[ -f /usr/bin/make ]] && echo N||echo Y;) ; then do_install_modules; fi

if ask "Do you want to download and extract the Linux kernel?" $([[ -f $D/kernel/.config ]] && echo N||echo Y;) ;then do_extract_kernel;fi

if [[ -f $D/kernel/.config ]]; then
  if ask "Do you want to run Menu Config ?" N ;then do_make_menuconfig; fi
  if ask "Do you want to compile the Linux kernel?" N ;then do_compile_kernel; fi
fi

if ask "Do you want to extract BZROOT" $([[ -L $D/bzroot/init ]] && echo N||echo Y;); then do_extract_bzroot ; fi

if [[ -L $D/bzroot/init ]]; then
  if [[ -f $D/kernel/.config ]]; then
    if ask "Do you want to slipstream compiled modules?" N ;then do_copy_modules; fi
  fi
  if ask "Do you want to install packages from /boot/extra ?" N ;then do_install_packages; fi
  if ask "Do you want to compress bzroot?" N ;then do_compress_bzroot; fi
fi

if [[ -f ${D}/bzroot-new ]]; then
  if ask "Do you want to copy ${D}/bzroot-new to ${BZROOT}-new?" N ;then do_copy_bzroot; fi
fi

if [[ -f $D/kernel/arch/x86/boot/bzImage ]]; then
  if ask "Do you want to copy bzimage to /boot/bzimage-new?" N ;then do_copy_bzimage; fi
fi
